<div style="margin-top: 20px; width: max( calc( 100% - 550px ), 400px ); max-width: calc( 100% - 40px ); height: auto; min-height: 480px; float: left; clear: none; ">
    ...
    while ($row = $stmtData->fetchObject()) {
        $row = (array)$row; ?>
        <a href="/schueler/<?php echo htmlspecialchars($row['id']); ?>"
           style="color: black; text-decoration: none; ">
            <div class="schueler_container">
                <div style="height: calc( 100% - 60px ); width: 100%; margin-top: 10px; " class="centriert">
                    <img loading="lazy" style="width: 130px; height: 130px; border-radius: 50%; "
                         src="<?php echo htmlspecialchars(empty($row['avatar']) ? "/resources/images/avatar.png" : $row['avatar']); ?>">
                </div>
                    ...
                    <div style="width: 100%; text-align: center; " class="centriert">
                        <div style="text-align: center; width: auto; height: auto; ">
                            ...
                            <a style="color: black; " href="javascript:void(0); "
                               onclick="event.stopPropagation(); vote('<?php echo htmlspecialchars($row['id']); ?>');"><?php } ?>
                                <div class="schueler_vote_count_<?php echo htmlspecialchars($row['id']); ?>"
                                     style="float: left; color: black; "><?php echo $row['rating']; ?></div>
                ...
                <?php if (isset($_SESSION['pupil']) && $pupil_data['activated'] == 1 && $row['id'] != $pupil_data['id']) { ?>
                    <div style="width: 100%; height: 25px; font-size: 14px; " class="centriert">
                        <button onclick="event.stopPropagation(); vote('<?php echo htmlspecialchars($row['id']); ?>');">
                            Gefällt Mir
                        </button>
                    </div>
                <?php } ?>
    ...
    <?php } ?>
    <button style="margin-top: 50px; width: 100%; height: 40px; background-color: darkslategray; color: white; "
            onclick="page_navigate('/schueler');">Alle Schüler anzeigen
    </button>
</div>
